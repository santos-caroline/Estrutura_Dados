...
class Time {
 private:  // Seção Privada: não é acessível fora da classe
  int hour;     // 0 - 23
  int minute;   // 0 - 59
  int second;   // 0 - 59
 
  public:   // Seção Pública: acessível fora da classe
  Time(int hour = 0, int minute = 0, int second = 0);         // =0 define valores padrão
  int getHour() const;   
