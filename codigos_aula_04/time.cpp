...
#include <iostream>  // Para usar a função std::cout.     < > para bibliotecas do sistema. 
#include "time.h"    // Para visualizar a classe Time.     " " para arquivos do projeto criado 
...
/* 
   Getters
 */
int Time::getHour() const {  // const se o método não vai alterar o atributo
  return hour;
}

int Time::getMinute() const {
  return minute;  // apenas retorna o atributo minute
}
...
/*
  Setters
 */
void Time::setHour(int hour) { // recebe hora como parâmetro e coloca no atributo
  this->hour = hour; // this->hour se refere ao atributo da classe, já que tem o mesmo nome do parâmetro. 
...
void Time::nextSecond() {
  second += 1; //acrescenta 1 s
  if (second >= 60) { //se passou 60 segundos

    second = 0;
    minute += 1; //acrescenta 1 min
  }
  if (minute >= 60) { //se passou 60 minutos
    minute = 0;
    hour += 1; //acrescenta 1 hora
  }
  if (hour >= 24) {  // se passou 24 horas
    // Reseta a hora para 0, já que o dia reinicia
    hour = 0;
  }
}
