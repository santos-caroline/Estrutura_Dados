...
Graph::Graph(int max, int null_edge) {
  NULL_EDGE   = null_edge; //atribuição
  maxVertices = max;  
  numVertices = 0;
  vertices = new Vertex[maxVertices]; //alocação de vetor de vértice
  marks = new bool[maxVertices]; //alocação do vetor para as marcações booleanas
  ...
  for (int row = 0; row < maxVertices; row++)
    for (int col = 0; col < maxVertices; col++)
      edges[row][col] = NULL_EDGE; //indica arresta vazia 
}
...
Graph::~Graph() { //destrutor
  ...
  delete [] marks;
  for (int row = 0; row < maxVertices; row++){
    delete [] edges[row]; //desalocando todas as linhas da matriz
  }
  delete [] edges;
}
...
void Graph::addVertex(Vertex vertex){
  vertices[numVertices] = vertex; //add o vertice que veio por parametro - vertice que chegou por ultimo fica na ultima posição do vetor
  
  numVertices++; //incrementado pq chegou outro vértice
}
void Graph::addEdge(Vertex fromVertex,
  ...
  edges[row][col] = weight; //para grafor não direcionados 
...
int Graph::getWeight(Vertex fromVertex,
  ...
  int row = getIndex(fromVertex);
  int col = getIndex(toVertex);
  return edges[row][col]; //a linha e coluna acessada para retornar o peso ao úsuario
}

void Graph::getAdjacents(Vertex vertex,
			 Queue &adjVertices){
  int fromIndex;
  int toIndex; //interar sobre todos os vertices que estão no gráfico
